...
interface AccountDashboardProps {
  ...
  onLogout: () => void;
  t: (key: string) => string;
  onRemoveFromWishlist: (variantId: number) => void;
...
// --- ITEM WISHLIST ---
type WishlistUIItem = Product & { selectedVariantId: number };
...
const WishlistItem: React.FC<{
  ...
  onClick={(e) => {
    e.stopPropagation();
    const variantId = Number(item.selectedVariantId);
    if (!variantId) return; // seguridad
    onRemove(variantId);
  }}
        ...
        return (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
             ...
             {user.wishlist.length > 0 ? (
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 {(user.wishlist as any[]).map((item: any) => (
  <WishlistItem
    key={`${item.id}-${item.selectedVariantId}`}
    item={item as WishlistUIItem}
    onRemove={onRemoveFromWishlist}
    onAddToCart={(itm, qty) => onAddToCart(itm, undefined, undefined, qty)}
    t={t}
  />
))}
